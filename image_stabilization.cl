...
__kernel void image_stabilization_float4_image_rgba(__global uchar4* result_buffer, read_only image2d_t image_current, read_only image2d_t image_next, const int width_current,const int height_current,  int width_next, int height_next, int block_x, int block_y){
	...
	for (int h = idy; h < steps_height_current; h+=stride_y)
		...
		int index_x = w;
		int index_y = h;
			for (int h_n = 0; h < height_next; h_n+=block_y)
				for (int w_n = 0; w <  width_next; w_n+=block_x){
					float sum = 0.0f;
					for (int i = -part_block_y; i <= part_block_y; i++)
						for (int j = -part_block_x; j <= part_block_x; j++){
							const float4 current_image_block = read_imagef(image_current, (int2)(w + j + part_block_x, h + i + part_block_y)) * (float4)Gaussian_filter_xy(convert_float(i), convert_float(j), sigma);
